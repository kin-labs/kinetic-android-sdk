...
data class KinBinaryMemo internal constructor(
    ...
    data class Builder @JvmOverloads constructor(
        ...
        fun build(): KinBinaryMemo {
            ...
            return KinBinaryMemo(
                ...
                typeId!!,
                appIdx,
                Base64.encodeToString(foreignKeyBytesPadded, Base64.DEFAULT)
            )
        }
    }
    ...
    companion object {
        ...
        fun decode(bytes: ByteArray): KinBinaryMemo = with(bytes.subByteArray(0, 4).byteArrayToInt()) {
            ...
            KinBinaryMemo(
                ...
                typeId,
                appIdx,
                Base64.encodeToString(foreignKey, Base64.DEFAULT)
            )
        }
    }

    val foreignKeyBytes: ByteArray by lazy {
        Base64.decode(foreignKey, Base64.DEFAULT)
            .subByteArray(0, 29)
    ...
    override fun toString(): String {
        return "KinBinaryMemo(magicByteIndicator=$magicByteIndicator, " +
                "version=$version, " +
                "typeId=${typeId.value}, " +
                "appIdx=$appIdx, " +
